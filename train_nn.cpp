...
static void train(Net &net, std::vector <Sample> &data, int epochs, int batch, float lr) {
    Net g;
    g.W1.resize(net.W1.size());
    g.b1.resize(H1);
    g.W2.resize(net.W2.size());
    g.b2.resize(H2);
    g.W3.resize(net.W3.size());
    g.b3.resize(OUT);
    ...
    for (int ep = 0; ep < epochs; ++ep) {
        ...
        for (int bs = 0; bs < N; bs += batch) {
            ...
            std::fill(g.W1.begin(), g.W1.end(), 0);
            std::fill(g.b1.begin(), g.b1.end(), 0);
            std::fill(g.W2.begin(), g.W2.end(), 0);
            std::fill(g.b2.begin(), g.b2.end(), 0);
            std::fill(g.W3.begin(), g.W3.end(), 0);
            std::fill(g.b3.begin(), g.b3.end(), 0);
